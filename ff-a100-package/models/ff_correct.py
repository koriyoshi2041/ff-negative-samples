"""
Corrected Forward-Forward Implementation

Key fixes from the original Hinton / mpezeshki implementation:
1. Goodness = MEAN of squared activations (not sum)
2. Layer-by-layer training (train each layer to convergence before next)
3. Label embedding uses x.max() (not 1.0)
4. Full-batch or large-batch training (not mini-batch)
...
class FFLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Normalize input (L2 normalization per sample)
    ...
    def train_layer(self, x_pos: torch.Tensor, x_neg: torch.Tensor, 
                    num_epochs: int = 1000, verbose: bool = True) -> torch.Tensor:
        """
        Train this layer to convergence.
        
        This is the key difference from mini-batch training:
        - Train each layer for many epochs before moving to next
        - This allows proper greedy layer-wise training
        """
        ...
        for _ in iterator:
            ...
            # log(1 + exp(-g_pos + threshold)) + log(1 + exp(g_neg - threshold))
...
class FFNetwork(nn.Module):
    ...
    Key difference: Layer-by-layer greedy training,
    ...
    def train_greedy(self, x_pos: torch.Tensor, x_neg: torch.Tensor,
        ...
        This is how FF is supposed to be trained:
    ...
    def predict(self, x: torch.Tensor, num_classes: int = 10) -> torch.Tensor:
        ...
        Uses sum of goodness across all layers for final prediction.
        """
...
def get_mnist_loaders(train_batch_size: int = 50000, test_batch_size: int = 10000):
    ...
    CRITICAL: Use large batch sizes (ideally full dataset)!
    The original implementation uses batch_size=50000 (full training set).
    """
    transform = Compose([
        ToTensor(),
...
def train_ff_correct(model: FFNetwork, 
                     train_loader: DataLoader,
                     device: torch.device,
    ...
    # Create negative samples (shuffled labels)
    # IMPORTANT: Shuffle labels, not random wrong labels!
...
def main(epochs_per_layer: int = 1000, seed: int = 1234):
    """Run the correct FF implementation."""
    # Setup
    ...
    # Train
    train_results = train_ff_correct(model, train_loader, device, epochs_per_layer)
    ...
    print("\n" + "-"*60)
    print("Reference (Hinton paper): ~1.4% test error")

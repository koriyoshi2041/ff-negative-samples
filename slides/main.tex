...
\begin{frame}{The Solution: Contrastive without Coupling (CwC-FF)}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \textbf{CwC-FF}
            \begin{itemize}
                \item Input: $[image]$ only
                \item Positive: real images
                \item Negative: corrupted images
    ...
    \begin{block}{Corruption Strategies}
        \begin{itemize}
            \item Gaussian noise addition
            \item Random pixel shuffling
            \item Patch masking

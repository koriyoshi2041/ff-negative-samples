#!/usr/bin/env python3
"""
Layer Collaboration FF - Fast Version (200 epochs)
"""
...
print("="*70)
print("Layer Collaboration FF - Fast Experiment (200 epochs)")
print("="*70)
...
class FFNet(nn.Module):
    ...
    def train_standard(self, xp, xn, epochs=200):
        hp, hn = xp, xn
    ...
    def train_collab(self, xp, xn, epochs=200, gamma=0.1):
        for ep in range(epochs):
            with torch.no_grad():
                gp_all, gn_all = [], []
                hp, hn = xp, xn
            ...
            if (ep+1) % 50 == 0:
                print(f"  Epoch {ep+1}: loss={total_loss/len(self.layers):.4f}")
...
configs = [
    ...
    ("Layer Collab (γ=0.05)", "collab", 0.05),
...
for name, mode, gamma in configs:
    ...
    t0 = time.time()
    if mode == "standard":
        model.train_standard(x_pos, x_neg, epochs=EPOCHS)
    else:
        model.train_collab(x_pos, x_neg, epochs=EPOCHS, gamma=gamma)
    train_time = time.time() - t0
    
    train_acc = model.accuracy(x_train, y_train)
    test_acc = model.accuracy(x_test, y_test)
    
    print(f"\n  Train: {train_acc*100:.2f}% | Test: {test_acc*100:.2f}% | Time: {train_time:.1f}s")
...
print("-"*60)
baseline = results[0]['test_accuracy']
for r in results:
    delta = (r['test_accuracy'] - baseline) * 100
...
output = {
    ...
    'conclusion': {
        ...
        'improvement': max(r['test_accuracy'] for r in results if r['gamma'] > 0) - baseline
    }
}
...
print(f"\n✓ Saved to results/layer_collab_verified.json")
print(f"\nConclusion:")
print(f"  Baseline: {baseline*100:.2f}%")
print(f"  Best Collab: {output['conclusion']['best_collab_test_acc']*100:.2f}% (γ={output['conclusion']['best_gamma']})")
